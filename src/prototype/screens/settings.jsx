...
function PlatformSettings({ toast }) {
  const { CHANNELS } = GGG;
  const { Pill, Btn, ChannelBadge } = UI;
  return (
    <>
      ...
      <SettingCard title="Developer API Credentials (Drop-in)" sub="Configure developer API keys for live automation campaigns. Leave empty to run standard high-fidelity offline simulators.">
        <div className="col" style={{ gap: 12 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div className="field">
              <label>Meta Ads API Developer Token</label>
              <input type="password" placeholder="EAAW... (Enter Meta Page/Ad Account token)" className="input mono" defaultValue=""/>
            </div>
            <div className="field">
              <label>Meta Ad Account ID</label>
              <input placeholder="act_123456789" className="input mono" defaultValue=""/>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div className="field">
              ...
              <input type="password" placeholder="TEST-DEV-TOKEN..." className="input mono" defaultValue=""/>
            </div>
            <div className="field">
              <label>Google Ads Client Customer ID</label>
              <input placeholder="123-456-7890" className="input mono" defaultValue=""/>
            </div>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div className="field">
              <label>CarsForSale Polling API / FTP Key</label>
              <input type="password" placeholder="CFS_SECRET_POLLING_KEY..." className="input mono" defaultValue=""/>
            </div>
            <div className="field">
              <label>Backup Video Generator Key (Google Veo / Omni)</label>
              <input type="password" placeholder="AIzaSy... (Google Cloud API Key)" className="input mono" defaultValue=""/>
            </div>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div className="field">
              <label>Primary Video Generation Model</label>
              <select className="select" defaultValue="omni" style={{ border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: "8px 10px", background: "var(--surface)", color: "var(--text)", width: "100%" }}>
                <option value="omni">Google Omni Studio (High-resolution walkarounds)</option>
                <option value="veo">Google Veo Engine (Cinematic b-rolls)</option>
                <option value="local">Local Mock Compiler (Offline sandbox)</option>
              </select>
            </div>
          </div>

          <div style={{ borderTop: "1px solid var(--border)", paddingTop: 12, display: "flex", justifyContent: "flex-end" }}>
            <button className="btn primary" onClick={() => toast("API keys stored securely in settings!")}>
              ⚡ Save Developer Keys
            </button>
          </div>
        </div>
...
function ImportSettings({ toast }) {
  ...
  React.useEffect(() => {
    if (typeof window !== "undefined") {
      setWebhookUrl(`${window.location.origin}/api/inventory/webhook?secret=cfs_webhook_secret`);
    }
  }, []);
  ...
  return (
    <>
      <SettingCard title="CarsForSale Auto-Ingest & Scheduled Polling" sub="Automated cron sync and realtime stock alerts">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
          <div className="field">
            <label>Auto-Ingest Polling Interval</label>
            <select className="select" defaultValue="12h">
              <option value="6h">Every 6 Hours</option>
              <option value="12h">Every 12 Hours (Recommended)</option>
              <option value="24h">Daily (Midnight Cron)</option>
              <option value="manual">Manual Sync Only</option>
            </select>
          </div>
          <div className="field">
            <label>Realtime Ingestion Webhook Receiver</label>
