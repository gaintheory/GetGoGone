...
function CBRightSidebar({ tab, onTab, selected, onUpdateLayer, onDeleteLayer, sizeId, onSize, exportSizes, brand, onBrand, vars, onAI, onGenerateVariations, onSaveVersion, onExport }) {
  const tabs = [
  ...
  return (
    <aside style={{
      ...
      <div style={{ overflow: "auto", padding: 14, flex: 1 }}>
        {tab === "layer" && <LayerInspector selected={selected} onUpdateLayer={onUpdateLayer} onDeleteLayer={onDeleteLayer} brand={brand} vars={vars}/>}
      ...
      <div style={{ borderTop: "1px solid var(--border)", padding: 10, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
        <button onClick={onSaveVersion} className="cb-btn">
          <Icon.Plus size={12}/> Save version
        </button>
...
function LayerInspector({ selected, onUpdateLayer, onDeleteLayer, brand, vars }) {
  if (!selected) {
    return (
  ...
  return (
    <>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        ...
        {selected.type !== "bg" && (
          <button onClick={() => onDeleteLayer(selected.id)} className="cb-icon-btn" title="Delete (Del)">
            <Icon.Trash size={13}/>
          </button>
        )}
      </div>
      ...
      {(selected.type === "shape" || selected.type === "bg") && (
...
window.CBRightSidebar = CBRightSidebar;
