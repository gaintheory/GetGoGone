...
function LayerEl({ l, vars, vehicle, brand, selected, onSelect, onLayerChange, onSnapshot, interactive }) {
  ...
  const { substitute } = window.CreativeBuilderData;
  ...
  const interactProps = interactive && !isBg ? {
    className: `cb-layer ${selected ? "selected" : ""} ${dragging ? "dragging" : ""}`,
    onPointerDown: handlePointerDown,
  } : {};

  // Resize handles when selected
  const handles = (interactive && selected && !isBg) ? (
    <>
  ...
  switch (l.type) {
    ...
    case "photo": {
      const svg = vehicleSvg(vehicle.body, vehicle.palette);
      return (
        <div {...interactProps} style={{ ...baseBox, overflow: "hidden", background: "#cbd5e1" }}>
          <div style={{ width: "100%", height: "100%", pointerEvents: "none" }}
            dangerouslySetInnerHTML={{ __html: svg.replace('viewBox="0 0 200 130"', 'viewBox="0 0 200 130" preserveAspectRatio="xMidYMid slice" width="100%" height="100%"') }}/>
...
window.CBCanvas = CBCanvas;
window.LayerEl = LayerEl;
