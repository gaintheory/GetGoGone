function TestDrive({ nav, toast }) {
  const { VEHICLES, fmt$, fmtMi } = window.GGG;
  ...
  const [vehicleIds, setVehicleIds] = React.useState(["v-1428", "v-1402"]);
  const [expandedId, setExpandedId] = React.useState("v-1428");
  const [pickerOpen, setPickerOpen] = React.useState(false);
  const [liked, setLiked] = React.useState("");
  const [objection, setObjection] = React.useState("");
  const [search, setSearch] = React.useState("");

  const vehicles = vehicleIds.map(id => VEHICLES.find(v => v.id === id)).filter(Boolean);
  ...
  const futureMatches = criteria ? VEHICLES.filter(v =>
...
function VehicleCard({ v, index, expanded, onToggle, onRemove }) {
  const { fmt$, fmtMi } = window.GGG;
  const { Pill, VehicleThumb } = window.UI;
